...
export default function OrganizationView( { orgaData }: Readonly<OrganizationViewProps> ) {
    ...
    const { data: event } = useGetEventsByOrgaId( orgaData.id )
    ...
    return (
        <div className={ "w-full max-w-[800px] mb-12 mx-auto" }>
            <Button variant={ "link" } onClick={ scrollToView }>
                Upcoming Events
            </Button>
            ...
            <div id={ "events" } className={ "space-y-4 mx-auto my-12" }>
                <div className={ "space-y-4" }>
                    <Text asTag={ "h2" } styleVariant={ "h2" } className={ "" }>
                        Upcoming Events
                    </Text>
                    <Separator className={ "" }/>
                </div>
                {
                    event?.data.length === 0 &&
                    <Text className={ "text-center" }>No upcoming events for this organization.</Text>
                }
                {
